...
if [ -f ${data}/${out_scp} ]; then
  mkdir -p ${data}/.backup
  echo "$0: moving $data/${out_scp} to $data/.backup"
  mv ${data}/${out_scp} ${data}/.backup
fi
...
if [ -f ${data}/segments ]; then
    ...
    ${cmd} JOB=1:${nj} ${logdir}/make_fbank_${name}.JOB.log \
        compute-fbank-feats-torchaudio.py \
            --fs ${fs} \
            --fmax ${fmax} \
            --fmin ${fmin} \
            --n_fft ${n_fft} \
